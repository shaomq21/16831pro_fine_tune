...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _process_proprio_features(self, projected_patch_embeddings, proprio, proprio_projector):
        ...
        if proprio_projector is not None and proprio is not None:
            ...
            proj_device = next(proprio_projector.parameters()).device
            patch_device = projected_patch_embeddings.device
            if proprio.device != proj_device:
                proprio = proprio.to(proj_device)
            proprio_features = proprio_projector(proprio)  # (bsz, llm_dim)
            if proprio_features.device != patch_device:
                proprio_features = proprio_features.to(patch_device)
