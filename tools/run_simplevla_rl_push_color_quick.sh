#!/usr/bin/env bash
# Quick SimpleVLA-RL on LIBERO goal task5 (push plate) + color OOD.
# Saves LoRA adapter only (no merge). Goal: baseline SR mid/low -> RL SR up.
set -uo pipefail
...
CKPT_PATH="${CKPT_PATH:-${STORAGE}/runs/simplevla_rl_push_color}"
NOTE="${NOTE:-$(date +%Y%m%d_%H%M%S)}"
EXPERIMENT_NAME="push_color_lora_${NOTE}"
NUM_GPUS="${NUM_GPUS:-2}"
GPU="${GPU:-3,4}"
DATASET_NAME="libero_goal"
TASK_ID="${TASK_ID:-5}"  # push the plate
...
# Prefer console logging; wandb may be broken in env
...
# Patch align.json for offline wandb
python3 - <<PY
import json
p="${ALIGN_PATH}"
d=json.load(open(p))
...
echo "===== $(date -Iseconds) SimpleVLA-RL quick start =====" | tee "${LOG}"
echo "SFT=${SFT_MODEL_PATH}" | tee -a "${LOG}"
echo "OUT=${CKPT_PATH}/RL/${EXPERIMENT_NAME}" | tee -a "${LOG}"
echo "GPU=${GPU} task_id=${TASK_ID} color=1" | tee -a "${LOG}"
